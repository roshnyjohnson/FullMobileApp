...
def generate_pdf_report(data: dict, timeframe_str: str, start_date: datetime, end_date: datetime) -> bytes:
    ...
    pdf.cell(0, 10, f"Period: {timeframe_str.capitalize()}", ln=True, align='C')
    ...
    pdf.cell(0, 8, f"Total Crowd Readings Recorded: {total_readings}", ln=True)
    ...
    if not readings and not alerts:
        ...
        for zid in sorted(list(zones)):
            z_name = zone_map.get(zid, f"Zone {zid}")
            ...
            if z_alerts:
                ...
                for risk, count in risk_counts.items():
                    pdf.cell(0, 8, f"    - {risk}: {count} occurrences", ln=True)
            else:
